...
bool j1App::Awake()
{
	...
	//H2 TODO 1 - Look at config.xml, how to create an xml file and what it does
	/*
	Anatomy of XML

		Prologue -	< ? xml version = ”1.0” encoding = ”iso - 8859 - 1” ? >
				< ? xml - declares where document starts
			version = ”X.Y” recommended version of XML to be evaluated
			encoding = ”iso - 8859 - 1” standarized character set that is use for the markup and content

		Content - Elements, attributes and entities(not much)
			Element - <element_name>content< / element_name>
				Case sensitive, names can’t contain <>&”.
...
*/
	...
	//H2 TODO 2: Create two new variables from pugui namespace:
	...
	//H2 TODO 3: Load "config.xml" file to a buffer, then send the data to
	...
	//This Ric way, loads directly config, no loadbuffer, we consider it safe fro direct use

	config = LoadConfig(config_file); //loads doc to node

	if(config.empty() == false) //checks if loaded
	{
		// self-config, proceeds to load info required I guess. We load all tags required here?
	...
	if(ret == true)
	{
		...
		while(item != NULL && ret == true)
		{
			...
			//H2 TODO 7: Add a new argument to the Awake method to receive a pointer to a xml node.
