...
SDL_Texture* const j1Textures::Load(const char* path)
{
	...
	//H1 TODO 5: Instead of reading directly from HD, use our new load methods from filesystem module
	// You will need to use IMG_Load_PNG_RW() instead
	//SDL_Surface* surface = IMG_Load(path);

	SDL_Surface* surface = IMG_Load_RW(App->fs->Load(path), 1); //Gets file from path (new filsystem) and loads it
	...
	{
		texture = LoadSurface(surface); //actually saves and puts texture
		SDL_FreeSurface(surface); //free surface (placeholder for texture)
	}
