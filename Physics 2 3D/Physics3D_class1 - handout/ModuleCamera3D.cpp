...
update_status ModuleCamera3D::Update()
{
	...
	if(App->input->GetMouseButton(SDL_BUTTON_RIGHT) == KEY_REPEAT)
	{
		...
		rotate(Reference, dx , Y);
		rotate(Reference, dy , X);
		LookAt(Reference);

		LOG("%f %f %f", Reference.x, Reference.y, Reference.z);

	}

	// Look at reference axis
	//if(App->scene_intro->primitive_list.count() > 0)
		//LookAt(App->scene_intro->primitive_list.getFirst()->data->transform.translation());

	//Look(this->Position, App->scene_intro->primitive_list.getFirst()->data->transform.translation());
